...
class PARS:
    ...
    def __init__(self, SIGMA_E):
        ...
        self.C_F = 150
        self.R = 0.02
        self.GAMMA = 1 / (1+self.R)
        self.D_CR = 0


        # initial model parameters in the log space
        self.SIGMA_D_0 = 20.85
        self.SIGMA_K_0 = 1
        self.MU_D_0 = -132.64
        self.MU_K_0 = 6.4
        self.DELTA_D = 10.5
        ...
        self.SIGMA_K_t_PRIOR = np.full(shape=self.T_END+1, fill_value=self.SIGMA_K_0)
        self.SIGMA_K_t_POST  = np.zeros(self.T_END+1)
        self.SIGMA_D_t_PRIOR = np.zeros(self.T_END+1)
        self.SIGMA_D_t_POST  = np.zeros(self.T_END+1)
        self.RHO_t_PRIOR     = np.zeros(self.T_END+1)
        self.RHO_t_POST      = np.zeros(self.T_END+1)

        # at timestep 1: first observation -> posterior values at 0 equal to prior
        self.SIGMA_K_t_POST[0]  = self.SIGMA_K_0
        self.SIGMA_D_t_PRIOR[0] = self.SIGMA_D_0
        self.SIGMA_D_t_POST[0]  = self.SIGMA_D_0
...
def A3_ana(e=1e3):
    if e<1e2:
        print("Warning: this analytical solution is based on low correlation between D & K -> we need high e!!")
